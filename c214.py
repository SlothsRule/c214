...
def lidar_data(point_cloud, lidar_point_cloud_buffer):
    matrix_rep_data = np.copy(np.frombuffer(point_cloud.raw_data, dtype=np.dtype('f4')))
    reshape_of_image = np.reshape(matrix_rep_data, (int(matrix_rep_data.shape[0] / 4), 4))
...
def generate_lidar_blueprint(blueprint_libary):
    lidar_blueprint = blueprint_libary.find('sensor.lidar.ray_cast')
    lidar_blueprint.set_attribute('channels', str(32))
    lidar_blueprint.set_attribute('points_per_seccond', str(50000))
